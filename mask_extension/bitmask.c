...
#define DEG_TO_RAD .01745329251994329
...
bitmask_t *bitmask_create(int w, int h, int fill)
{
  bitmask_t *temp;
...
bitmask_t *bitmask_scale(const bitmask_t *m, int w, int h)
{
  ...
  if (w < 1 || h < 1)
    {
      nm = bitmask_create(1,1, 0);
      return nm;
    }

  nm = bitmask_create(w,h, 0);
  if (!nm)
    return NULL;
  ny = dny = 0;
...
bitmask_t* bitmask_copy(const bitmask_t *mask) {
    bitmask_t *new = NULL;
    size_t size = offsetof(bitmask_t,bits) + mask->h*((mask->w - 1)/BITMASK_W_LEN + 1)*sizeof(BITMASK_W);
    
    if ((new = malloc(size)) == NULL) {
        return NULL;
    }
    
    memcpy(new, mask, size);
