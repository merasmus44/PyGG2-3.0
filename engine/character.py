...
class Character(entity.MovingObject):
    ...
    def step(self, game, state, frametime):
        ...
        # this is quite important, if hspeed / 20 drops below 1 self.animoffset will rapidly change and cause very
        # fast moving legs (while we are moving very slow)
    ...
    def endstep(self, game, state, frametime):
        ...
        if state.map.collision_mask.overlap(self.collision_mask, (int(round(self.x)), int(round(self.y)))):
            ...
            if onground and not state.map.collision_mask.overlap(self.collision_mask,
                                                                 (int(round(self.x)), int(round(self.y - 6)))):
            ...
            elif onground and not state.map.collision_mask.overlap(self.collision_mask, (
            int(round(self.x)), int(round(self.y - 12)))) and state.map.collision_mask.overlap(self.collision_mask, (
            int(round(self.x - 6 * function.sign(self.hspeed))), int(round(self.y)))):
                while state.map.collision_mask.overlap(self.collision_mask, (int(round(self.x)), int(round(self.y)))):
                    self.y -= 1
            else:
                if self.hspeed < 0:
    ...
    def serialize(self, state, packetbuffer):
        ...
        # Serialize intel, doublejump, etc... in one byte. Should we merge this with the input serialization in
        # Player? Move the input ser. here?
        byte = 0
        byte |= self.intel << 0
